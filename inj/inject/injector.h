...
/////////////////////////////////
void payson1337(LPCSTR window_class_name, LPCWSTR dll_path)
{
	system("cls");
	// get dll file
	...
	{		
		...
		{
			driver().free_memory_ex(allocate_base);
			printf(xor_a(" Failed to Import\n"));
		}

		printf(xor_a(" Successfully Imported\n"));
		...
		printf(xor_a(" Successfully Called DLL.\n"));

		// cleanup
		erase_discardable_sect(allocate_base, dll_nt_head);
		VirtualFree(dll_image, 0, MEM_RELEASE);

		printf(xor_a(" Successfully Injected DLL.\n"));
		Sleep(3000);
		system("cls");
		system("color 5");
		printf(xor_a("\n Made By Payson Holmes - github.com/paysonism")); // remove this = gay
		Sleep(3000);
		exit(0);
	}
	else
	{
		printf(xor_a(" 'FortniteClient-Win64-Shipping.exe' not found. Please ensure that nova is open and in lobby.\n"));
	}
}
/////////////////////////////////
